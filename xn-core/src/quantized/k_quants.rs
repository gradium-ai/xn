...
impl GgmlType for BlockQ8_0 {
    ...
    fn vec_dot_unopt(n: usize, xs: &[Self], ys: &[Self::VecDotType]) -> Result<f32> {
        ...
        for (xs, ys) in xs.iter().zip(ys.iter()) {
            let sum_i =
                xs.qs.iter().zip(ys.qs.iter()).map(|(&x, &y)| x as i32 * y as i32).sum::<i32>();
            sumf += sum_i as f32 * f16::to_f32(xs.d) * f16::to_f32(ys.d)
        }
        Ok(sumf)
    }
...
impl GgmlType for BlockQ2K {
    ...
    fn to_float(xs: &[Self], ys: &mut [f32]) -> Result<()> {
        for (block, y) in group_for_dequantization(xs, ys)? {
            ...
            for (y_block, qs) in
                y.as_chunks_mut::<128>().0.iter_mut().zip(block.qs.as_chunks::<32>().0)
            {
                // Step by 32 over q.
                let mut shift = 0;
...
impl GgmlType for BlockQ3K {
    ...
    fn vec_dot_unopt(n: usize, xs: &[Self], ys: &[Self::VecDotType]) -> Result<f32> {
        ...
        for (x, y) in xs.iter().zip(ys.iter()) {
            ...
            for _ in 0..QK_K / 128 {
                ...
                a.iter_mut().take(32).zip(hmask).for_each(|(a_val, hmask_val)| {
    ...
    fn from_float(xs: &[f32], ys: &mut [Self]) -> Result<()> {
        for (block, x) in group_for_quantization(xs, ys)? {
            ...
            for j in 0..QK_K / 16 {
                let sc = if j < 8 { block.scales[j] & 0xF } else { block.scales[j - 8] >> 4 };
    ...
    fn to_float(xs: &[Self], ys: &mut [f32]) -> Result<()> {
        ...
        for (block, y) in group_for_dequantization(xs, ys)? {
            ...
            for (y, qs) in y.as_chunks_mut::<128>().0.iter_mut().zip(block.qs.as_chunks::<32>().0) {
                let mut shift = 0;
                for shift_scoped_y in y.as_chunks_mut::<32>().0 {
                    for (scale_index, scale_scoped_y) in
                        shift_scoped_y.as_chunks_mut::<16>().0.iter_mut().enumerate()
                    {
...
impl GgmlType for f32 {
    ...
    fn vec_dot_unopt(n: usize, xs: &[Self], ys: &[Self::VecDotType]) -> Result<f32> {
        ...
        let res = xs[..n].iter().zip(ys[..n].iter()).map(|(&x, &y)| x * y).sum::<f32>();
        Ok(res)
    }
