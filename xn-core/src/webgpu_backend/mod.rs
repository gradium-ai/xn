...
//! share one compute pass, which execute in order and are required by WebGPU to
//! observe their predecessors' writes; a buffer copy cannot sit inside a pass,
//! so recording one closes the pass first. The flush waits for GPU completion
//! via `Device::poll`.
...
/// Entries kept in the bind-group cache before it is dropped wholesale. A
/// decode step records ~800 dispatches, and which pooled buffer each
/// intermediate lands in varies between steps, so the working set of distinct
/// (pipeline, buffers) tuples is a few thousand -- 8192 thrashed. Each entry is
/// a small handle and the buffers it pins live in the pool regardless.
...
///
/// The identity is what makes the bind-group cache possible: `wgpu::Buffer` is
/// only `Clone + Debug`, with no equality or hash, so there is nothing to key a
/// cache on. Ids are handed out once per *created* buffer and travel with it
/// through the recycling pool, so a bind group built for a given tuple of
/// buffers stays valid for as long as those buffers exist -- which is forever,
/// since pooled buffers are reused rather than destroyed.
///
/// Derefs to `wgpu::Buffer` so call sites that just need the buffer are
/// unchanged.
struct Buf {
    buffer: wgpu::Buffer,
    class: u64,
...
struct OpCtx {
    ...
    /// The compute pass dispatches are being recorded into. Held open across
    /// consecutive dispatches -- see `compute_pass`. Must be dropped (via
    /// `end_pass`) before the encoder is touched again or finished.
...
pub struct DeviceInner {
    ...
    /// Bind groups keyed by (pipeline index, buffer ids). Rebuilding one per
    /// dispatch was the second half of this backend's CPU-side cost after the
    /// per-operator compute pass. Which pooled buffer each intermediate lands
    /// in shifts from step to step, so this is not a perfect cache: it measures
    /// ~74% hits on a decode step (`XN_WEBGPU_PROFILE=1` reports the rate).
...
impl Device {
    ...
    fn dispatch_nd(
        ...
        let bind_group = match bgs.get(&(pidx, key)) {
            Some(bg) => {
                if self.profile {
                    self.pstats.lock().unwrap().bg_hits += 1;
                }
                bg.clone()
            }
            None => {
                if self.profile {
                    self.pstats.lock().unwrap().bg_misses += 1;
                }
                ...
                // A bind group keeps its buffers alive, which is harmless (they
                // live in the pool anyway), but the number of distinct tuples is
                // not bounded a priori, so cap the cache.
    ...
    /// The compute pass dispatches record into, opening one if needed.
    ///
    /// A pass is held open across consecutive dispatches: opening one per
    /// operator is what made this backend spend 38% of its wall-clock building
    /// commands with the GPU idle, and on Metal it means a fresh
    /// `MTLComputeCommandEncoder` per operator. The native Metal backend keeps
    /// one serial encoder open for a whole batch and the Vulkan backend records
    /// into one command buffer; this is the WebGPU equivalent.
    ///
    /// Ordering is still correct: dispatches within a pass execute in order and
    /// WebGPU requires the implementation to make each one observe the writes of
    /// its predecessors (there are no explicit barriers to insert), which is the
    /// same guarantee the Metal backend gets from `MTLDispatchType::Serial`.
    fn ensure_pass(&self, ctx: &mut OpCtx) {
        ...
        if ctx.pass.is_none() {
            ...
            // The encoder must not be touched while the pass is alive, which
            // `end_pass` (called before every copy and before `finish`)
            // guarantees.
