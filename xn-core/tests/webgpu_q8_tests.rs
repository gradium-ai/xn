...
#[test]
fn q8_matmul_f16() -> Result<()> {
    if !wg().supports_f16() {
        eprintln!("skipping: adapter does not advertise WGSL shader-f16");
        return Ok(());
    }
    for (i, &(m, n, k)) in CASES.iter().enumerate() {
        // f16-representable inputs, so the only f16 error is the output store
        // and the activations' own rounding is not double-counted.
        let q16 = |v: Vec<f32>| -> Vec<f32> {
            v.into_iter().map(|x| f16::from_f32(x).to_f32()).collect()
        };
        let xs = q16(rnd(i as u64 + 1, m * k, -1.0, 1.0));
        // The var builder loads weights in dtype T, so for T = f16 the weight is
        // already f16 before it is quantized. Rounding here too keeps the
        // reference measuring the kernel rather than that extra rounding, which
        // can flip a quant by one step and shows up amplified over k terms.
        let w = q16(rnd(i as u64 + 500, n * k, -1.0, 1.0));
        let wdq = dequantized(&w, n, k);
        for bias in [None, Some(q16(rnd(i as u64 + 900, n, -0.5, 0.5)))] {
            let b = bias.as_deref();
            let want = reference(&xs, &wdq, b, m, n, k);
            let got = run_q8::<f16, Q80F16>(&xs, &w, b, m, n, k);
            cmp(&format!("q8 f16 {m}x{n}x{k} bias={}", b.is_some()), &want, &got, 5e-3, 5e-3);
        }
    }
    Ok(())
}
