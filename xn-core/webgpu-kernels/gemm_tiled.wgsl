// Tiled GEMM with a 4x4 register tile (f32 accumulation), general strides + batch.
...
//
// ---------------------------------------------------------------------------
// Structure ported from MLC-LLM / WebLLM's generated prefill matmul.
//
// The previous version of this kernel was a 16x16 shared-memory tile computing
// ONE output per thread: two global loads bought sixteen multiply-adds, so it
// was memory-bound on a compute-bound problem, and it measured ~9x slower than
// WebLLM at prefill on the same GPU.
//
// This follows the shape TVM's dlight matmul schedule emits for WebGPU, read
// out of WebLLM's own compiled artifact (`NT_matmul_kernel_2`, 33 KB of
// unrolled WGSL): a 64-thread (8x8) workgroup, a 32x32 output tile, a 4x4
// register tile per thread (`array<f32, 16>` accumulators), and operand tiles
// staged 8-deep in workgroup memory (2 x 256 f32).
//
// Origin, Apache-2.0:
//   schedule  https://github.com/apache/tvm/blob/main/python/tvm/s_tir/dlight/gpu/matmul.py
//             (was python/tvm/dlight/gpu/matmul.py through TVM v0.21.0)
...
// The WGSL below is written from that structure, not copied token-for-token:
// dlight specializes every shape at compile time and fully unrolls, while this
// kernel stays general over m/n/k and arbitrary strides.
// ---------------------------------------------------------------------------
struct Params {
...
fn main(
    ...
    //
    // Deliberately NOT `array<f32, 16>` with loop indices: naga cannot keep a
    // dynamically indexed function-scope array in registers, so it spills to
    // thread-private memory and the kernel measured 2.3x SLOWER than the old
    // one-output-per-thread version. dlight sidesteps this by fully unrolling
    // every specialized shape; this does the same by hand.
    var c00 = 0.0;
    var c01 = 0.0;
    var c02 = 0.0;
    var c03 = 0.0;
    ...
    for (var kt = 0u; kt < nk; kt = kt + 1u) {
        ...
        //
        // Two paths. dlight specializes every shape at compile time, so its
        // generated kernel has no bounds checks at all; the price of staying
        // general is a guard per staged element. `interior` recovers the
        // bounds-free inner loop for every tile that is fully in range, which
        // is all of them except the last row/column/k-slice.
        if interior {
            ...
            for (var s = 0u; s < 4u; s = s + 1u) {
                ...
                // at[r][kk]: consecutive threads walk kk, which is contiguous in
                // memory whenever lhs is row-major (lhs_cs == 1).
                let ar = idx / KSTEP;
                let ak = idx % KSTEP;
                let grow = row0 + ar;
                let gk = k0 + ak;
                ...
                // bt is always laid out [kk][c] for the compute loop, but which
                // global element a thread fetches depends on how rhs is stored:
                // consecutive threads must walk whichever axis is contiguous.
                //   rhs_rs == 1 (matmul_t -- every linear layer): k is contiguous,
                //     so walk k and scatter into bt.
                //   otherwise (the attention matmuls): columns are contiguous.
                var bk: u32;
                var bc: u32;
                if pc.rhs_rs == 1u {
                    bc = idx / KSTEP;
        ...
        // Eight loads buy sixteen multiply-adds, against two loads for one in
        // the old kernel.
