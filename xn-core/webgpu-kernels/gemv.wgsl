...
//
// Ported to this branch's uniform-parameter and templated-scalar conventions;
// accumulation stays f32 whatever S is.
//
// Origin, Apache-2.0:
...
fn main(
    ...
    if vec_ok {
        ...
        for (var g = tid; g < k4; g = g + TPB) {
            let l = g * 4u;
            let a0 = f32(lhs[lbase + (l + 0u) * pc.lhs_cs]);
            let a1 = f32(lhs[lbase + (l + 1u) * pc.lhs_cs]);
            let a2 = f32(lhs[lbase + (l + 2u) * pc.lhs_cs]);
            let a3 = f32(lhs[lbase + (l + 3u) * pc.lhs_cs]);
            let w0 = vec4<f32>(rhs4[v0 + g]);
            let w1 = vec4<f32>(rhs4[v1 + g]);
            let w2 = vec4<f32>(rhs4[v2 + g]);
            let w3 = vec4<f32>(rhs4[v3 + g]);
        ...
        for (var l = kbulk + tid; l < pc.k; l = l + TPB) {
            let a = f32(lhs[lbase + l * pc.lhs_cs]);
            acc0 = fma(f32(rhs[rb0 + l]), a, acc0);
            acc1 = fma(f32(rhs[rb1 + l]), a, acc1);
            acc2 = fma(f32(rhs[rb2 + l]), a, acc2);
            acc3 = fma(f32(rhs[rb3 + l]), a, acc3);
        }
    } else {
        for (var l = tid; l < pc.k; l = l + TPB) {
            let a = f32(lhs[lbase + l * pc.lhs_cs]);
            let lo = l * pc.rhs_rs;
            acc0 = fma(f32(rhs[rb0 + lo]), a, acc0);
            acc1 = fma(f32(rhs[rb1 + lo]), a, acc1);
            acc2 = fma(f32(rhs[rb2 + lo]), a, acc2);
            acc3 = fma(f32(rhs[rb3 + lo]), a, acc3);
        }
    }
    ...
    if tid < TN {
        let jj = j0 + tid;
        if jj < pc.n {
            dst[b * pc.n + jj * pc.dst_cs] = S(sh[tid]);
        }
    }
}
